        (function () {
            ...
            function tgCheckSuccess() {
                ...
                // 고정 800×600 해상도로 비교 (모바일·PC 동일 정밀도)
                ...
                tgShapes.forEach(sh => drawShape(shpCtx, sh));
                const shpData = shpCtx.getImageData(0, 0, COMP_W, COMP_H).data;

                // ── 3. 픽셀 집계 ─────────────────────────────────
                let silPx = 0, errPx = 0, covPx = 0;
                ...
                // ── 조건 A: XOR 오차 < 5% ─────────────────────────
                // 5%: 정확한 배치에서도 렌더링 아티팩트(안티앨리어싱) 허용
                // 개별도형 90% 조건 C가 실제 위치 정밀도를 보장
                const errRatio = silPx > 0 ? errPx / silPx : 1;
                if (errRatio >= 0.05) return;

                // ── 조건 B: 도형 간 겹침 < 4% ─────────────────────
                // Shoelace 면적 공식 (이동·회전 불변)
                ...
                let totalIndivPx = 0;
                tgShapes.forEach(sh => { totalIndivPx += polyArea(sh.points); });
                const overlapPx = Math.max(0, totalIndivPx - covPx);
                const overlapRatio = silPx > 0 ? overlapPx / silPx : 1;
                if (overlapRatio >= 0.04) return;

                // ── 조건 C: 개별 도형 각각 85%+ 실루엣 내부 ────────
                // 핵심: 회전 잘못된 도형이 실루엣 바깥으로 삐져나오면 차단
                for (const sh of tgShapes) {
                    const pieceCanvas = document.createElement('canvas');
                    pieceCanvas.width = COMP_W; pieceCanvas.height = COMP_H;
                    const pieceCtx = pieceCanvas.getContext('2d');
                    drawShape(pieceCtx, sh);
                    const pieceData = pieceCtx.getImageData(0, 0, COMP_W, COMP_H).data;

                    let piecePx = 0, pieceInSilPx = 0;
                    for (let i = 3; i < pieceData.length; i += 4) {
                        if (pieceData[i] > 128) {
                            piecePx++;
                            if (silData[i] > 128) pieceInSilPx++;
                        }
                    }
                    // 도형 면적의 90% 이상이 실루엣 안에 있어야 함 (85%→90% 강화)
                    const insideRatio = piecePx > 0 ? pieceInSilPx / piecePx : 0;
                    if (insideRatio < 0.90) return; // 이 도형이 실루엣 밖으로 삐져나옴 → 거부
                }
